...
def ProgressiveHedging(x_t, future_t):
        ...
        for k in range(max_iter_num):
                ...
                for index, scenario in enumerate(scenarios_list):
                        opt_sol, opt_state = solve_lagrange_sub_problem(epsilons=scenario, A=A, x_0=x_t,w_s=w[index],r=r,last_policy=implementable_policy) 
                        opt_sol_for_each_scenario.append(opt_sol)
                opt_sol_for_each_scenario = np.array(opt_sol_for_each_scenario)
                # early stop
        ...
        for t in range(0,num_t):
                if t == 0 :
                        ...
                        for x_t in x_dict[t]:
                                ...
                                for epsilon in epsilon_element:
                                        x_dict[t+1].append(x_t * A + implementable_policy[0] + epsilon)
...
def lqr_dp(A,B,Q,Q_f,R,N):
        ...
        for t in range(N,0,-1):
                P_list[t-1] = Q + A*P_list[t]*A - A*P_list[t]*B*1/(R+B*P_list[t]*B)*B*P_list[t]*A
...
# calculate K_t
for t in range(0, num_t):
        print("K_"+str(t),":", [round(x,4) for x in np.array(optimal_policy[t]) / np.array(optimal_x[t])])
